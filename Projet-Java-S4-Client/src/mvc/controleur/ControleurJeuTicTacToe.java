...
public class ControleurJeuTicTacToe implements Initializable {
	 ...
	 *
	 * @Override public Void call() { try { if (intTtt.getNombreJoueur(idPartie) ==
	 * 1) { tourJoueur.setText("En attente d'un deuxième joueur.."); numJoueur = 1;
	 * while (intTtt.getNombreJoueur(idPartie) == 1) {
	 * System.out.println("en attente.."); TimeUnit.SECONDS.sleep(2); } } else {
	 * numJoueur = 2; } labelJoueur.setText("Vous êtes le joueur " + numJoueur);
	 * System.out.println("JOUEUR " + numJoueur);
	 *
	 * tourJoueur.setText("Au tour du joueur " + intTtt.getTourActuel(idPartie));
	 *
	 * intTtt.setTourActuel(idPartie, tour); } catch (RemoteException |
	 * InterruptedException e) { System.out.println(e); } return null; } };
	 */

	/*
	 * public void tour() { try { labels = intTtt.getLabels(idPartie);
	 *
	 * label1.setText(labels[0]); label2.setText(labels[1]);
	 * label3.setText(labels[2]); label4.setText(labels[3]);
	 * label5.setText(labels[4]); label6.setText(labels[5]);
	 * label7.setText(labels[6]); label8.setText(labels[7]);
	 * label9.setText(labels[8]);
	 *
	 * System.out.println("Tour : " + intTtt.getTourActuel(idPartie) +
	 * ", Num joueur : " + numJoueur);
	 *
	 * if (intTtt.getTourActuel(idPartie) == 1 && numJoueur == 1 ||
	 * intTtt.getTourActuel(idPartie) == 2 && numJoueur == 2) {
	 * label1.setDisable(false); label2.setDisable(false); label3.setDisable(false);
	 * label4.setDisable(false); label5.setDisable(false); label6.setDisable(false);
	 * label7.setDisable(false); label8.setDisable(false); label9.setDisable(false);
	 * } else { label1.setDisable(true); label2.setDisable(true);
	 * label3.setDisable(true); label4.setDisable(true); label5.setDisable(true);
	 * label6.setDisable(true); label7.setDisable(true); label8.setDisable(true);
	 * label9.setDisable(true); }
	 *
	 * } catch (RemoteException e) { System.out.println("Tour exception " + e); } }
	 */

	/*
	 * public void poseForme(Label l) { try { String joueurActuel =
	 * "Au tour du joueur "; if (intTtt.getTourActuel(idPartie) == 1) {
	 * l.setText("X"); intTtt.setTourActuel(idPartie, tour + 1); } else {
	 * l.setText("O"); intTtt.setTourActuel(idPartie, tour - 1); } joueurActuel +=
	 * intTtt.getTourActuel(idPartie); tourJoueur.setText(joueurActuel); } catch
	 * (RemoteException e) { System.out.println("Tour exception " + e); } }
	 */
